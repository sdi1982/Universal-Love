...
//LPDIRECT3DTEXTURE9		g_pTexPig		= NULL;
LPDIRECT3DDEVICE9 g_pd3dDevice = NULL;

LmSceneManager g_LmSceneManager;
UINT g_company = 0;
UINT g_difficulty = 0;
ULONG g_money = 0;
UINT g_characterType = 0;
...
void InitApp()
{
    ...
    //g_HUD.AddButton( IDC_TOGGLEFULLSCREEN, L"Toggle full screen", 35, 10, 125, 25 );
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
	...
	// 씬 메니저 생성
	g_LmSceneManager.Load(&g_SampleUI,pd3dDevice);

	g_LmSceneManager.SetGameState(START_STATE);
	
	//if( FAILED(D3DXCreateTextureFromFile(pd3dDevice,L"start_button.png", &g_pTexPig)) )
	//	return false;
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
		...
		g_LmSceneManager.StateRender(g_pSprite);


		//SetRect(&rc, 0, 0, 128, 128);
		//D3DXVECTOR3 Pos2(325,300,0);
		//g_pSprite->Draw(g_pTexPig, &rc, NULL, &Pos2, D3DCOLOR_RGBA(255,255,255,255));
		//
		//	SetRect(&rc, 0, 0, 512, 256);
		//	g_pSprite->Draw(g_pTexPig, &rc, NULL, &Pos, D3DCOLOR_RGBA(255,255,255,255));
		//
		//	SetRect(&rc, 0, 0, 256, 512);
		//g_pSprite->Draw(g_pTexRabbit, &rc, NULL, &Pos, D3DCOLOR_RGBA(255,255,255,255));
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
	...
	g_LmSceneManager.GUIEvent(nControlID);
}
...
void CALLBACK OnDestroyDevice( void* pUserContext )
{
	...
	//SAFE_RELEASE(g_pTexPig);
	g_LmSceneManager.DestroyAll();
	g_LmSceneManager.ReleaseAll();
